...
def get_session():
    session = boto3.Session(
        aws_access_key_id=os.environ["AMI_OBJECT_STORE_ACCESS_KEY_ID"],
        aws_secret_access_key=os.environ["AMI_OBJECT_STORE_SECRET_ACCESS_KEY"],
    )
    return session
...
def put_file(deployment: str, local_path: pathlib.Path):
    """
    Upload a file to S3.

    :param deployment: str
    :param local_path: pathlib.Path
    """
    bucket = get_bucket()
    destination = with_trailing_slash(deployment) + local_path.name
    bucket.upload_file(str(local_path), destination)
    return destination
