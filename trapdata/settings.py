...
class Settings(BaseSettings):
    ...
    localization_model: str = Field(default=ml.models.DEFAULT_OBJECT_DETECTOR)
    binary_classification_model: str = Field(
        default=ml.models.DEFAULT_BINARY_CLASSIFIER
    )
    species_classification_model: str = Field(
        default=ml.models.DEFAULT_SPECIES_CLASSIFIER
    )
    feature_extractor: str = Field(default=ml.models.DEFAULT_FEATURE_EXTRACTOR)
    classification_threshold: float = 0.6
    localization_batch_size: int = 2
    classification_batch_size: int = 20
    num_workers: int = 1

    @field_validator("image_base_path", "user_data_path")
    ...
    @classmethod
    def customize_sources(cls, data: Any) -> Any:
        if isinstance(data, dict):
            data.update(kivy_settings_source(BaseSettings()))
        return data


class PipelineSettings(Settings):
    image_base_path: FilePath = Field(
        default=...
    )  # Override default settings to enforce image_base_path
