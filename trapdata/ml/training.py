...
def fetch_dataset(
    ...
    rows = [
        ...
        for detection_id, label, split, vector in zip(
            detection_ids, labels, splits, features
        )
    ]
    logger.info(
        f"Loaded {len(rows)} training rows over {len(classes)} species from {url}"
    )
    return rows, metadata


def _matrices(
    rows: list[TrainingRow], labels: list[str]
...
def warm_start(
    ...
    with torch.no_grad():
        for i, name in enumerate(labels):
            ...
            if source is None:
                continue
            head.weight[i] = torch.from_numpy(
                np.asarray(weights[source], dtype=np.float32)
            )
...
def restore_untrained_classes(
    ...
    if restored:
        logger.info(
            f"Restored {restored} class(es) that had no verified crops this run"
        )
    return restored
...
def retrain(
    ...
    restored = restore_untrained_classes(
        weights, bias, labels, trained_counts, incumbent
    )
...
def save_head(
    result: dict[str, typing.Any], directory: pathlib.Path, name: str
) -> dict[str, str]:
    """
